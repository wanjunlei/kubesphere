...
```bash
kubectl get extensionversions -n kubesphere-system -l kubesphere.io/extension-ref=whizard-notification -o jsonpath='{range .items[*]}{.spec.version}{"\n"}{end}' | sort -V | tail -1
```
...
```bash
kubectl get installplan -n kubesphere-system whizard-notification
kubectl get extension whizard-notification
kubectl get extensionversion whizard-notification-<VERSION>
```
...
```bash
kubectl delete installplan -n kubesphere-system whizard-notification
```
